...
void read(Points *point) {
	fstream fin;
	fin.open("src/pointsV1.1.csv", ios::in);
	string line;
	getline(fin, line);
	int count = 0;
...
int main() {
	...
	val += distance(point[1].getLatitude(), point[1].getLongitude(),
				point[2].getLatitude(), point[2].getLongitude());
	cout << "The Distance between "<< point[0].getAbbriviation() << ":" <<
			point[1].getAbbriviation() << ":" << point[2].getAbbriviation() <<
			" is " << val << " miles" << endl;
	return 0;
}
